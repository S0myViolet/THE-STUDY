  ...
  return (
    <>
      ...
      <nav aria-label="Primary" className="md:hidden fixed bottom-0 left-0 right-0 z-50 h-[64px] pb-[env(safe-area-inset-bottom)] bg-paper border-t border-line flex">
        <Tab href="/desk" label="Desk" icon={<I.Desk size={18} />} active={isActive("/desk")} onNavigate={() => setSheet(null)} />
        <Tab href="/casebook" label="Case" icon={<I.Case size={18} />} active={isActive("/casebook")} onNavigate={() => setSheet(null)} />
        <Tab label="Train" icon={<I.Eye size={18} />} active={trainActive || sheet === "train"} onClick={() => setSheet(sheet === "train" ? null : "train")} />
        <Tab href="/archive" label="Archive" icon={<I.Archive size={18} />} active={isActive("/archive")} onNavigate={() => setSheet(null)} />
